...
def test_core_and_domain_never_import_provider_memory_or_controller_adapters() -> None:
    ...
    for directory in ("core", "modules"):
        for path in (root / directory).rglob("*.py"):
            assert not any(name.startswith(("journeymap.adapters", "openai", "anthropic")) for name in imported_names(path))
    allowed = {"journeymap.core.canonical", "journeymap.core.observations", "journeymap.core.handlers"}
    for filename in ("human.py", "provider.py", "memory.py"):
        imports = {name for name in imported_names(root / "adapters" / filename) if name.startswith("journeymap")}
...
def test_provider_accepts_only_versioned_configuration_and_returns_raw_data() -> None:
    class FakeProvider:
        def generate(self, request: ProviderRequest) -> RawModelResponse:
            assert {f.name for f in fields(request)} == {
                "prompt", "prompt_version", "model", "configuration_version", "parameters", "schema_version",
            }
    ...
    assert asdict(response) == {"text": "unparsed", "metadata": {"provider_request_id": "fake-1"}, "schema_version": 1}
...
def test_memory_only_receives_ordered_same_actor_prior_observations_and_no_memory_is_empty() -> None:
    ...
    for prior in (
        (second, first), (first, first), (current,),
...
@pytest.mark.parametrize("private", ["missing-wallet", "invalid-wallet", "missing-stock", "invalid-stock"])
def test_private_seller_resource_shape_has_no_distinct_actor_diagnostic(private: str) -> None:
    ...
    try:
        ...
        receipt = game.submit(ActionRequest("buy", observation.run_id, "stranger", observation.observation_id, 0, "BUY", 1, {"offer_id": "edwin-bread", "quantity": 1}))
