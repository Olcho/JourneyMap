...
from journeymap.application.contracts import ACTION_V1_VALIDATORS, normalize_request, validate_action_contract
...
from journeymap.core.handlers import ActionRequest, ActionResult, ActionStatus, ActionValidationError
...
def sample(action: str, payload: JsonObject | None = None) -> ActionRequest:
    return ActionRequest("r", "run", "actor", "obs", 0, action, 1, EXAMPLES[action] if payload is None else payload)
...
def test_inform_optional_reply_and_request_nested_data_are_exact() -> None:
    ...
    for bad in (None, "", 1):
        with pytest.raises(ActionValidationError, match="INVALID_PAYLOAD"):
            validate_action_contract(sample("INFORM", {**EXAMPLES["INFORM"], "reply_to_event_id": bad}))
    payload: JsonObject = {
        **EXAMPLES["REQUEST"],
...
@pytest.mark.parametrize("bad", [float("nan"), float("inf"), float("-inf"), {1: 2}, (1,), {1}, object(), "\ud800"])
...
def test_cyclic_payload_and_non_object_envelope_fail_closed() -> None:
    ...
    for field, bad in (("payload", []), ("correlation_id", ""), ("submitted_at", True), ("schema_version", True)):
        request = sample("WAIT")
...
def test_action_request_and_receipt_golden_envelopes() -> None:
    assert asdict(sample("WAIT")) == {
        "action_request_id": "r", "run_id": "run", "actor_id": "actor",
        "based_on_observation_id": "obs", "submitted_at": 0, "action_type": "WAIT",
        "schema_version": 1, "payload": {"duration": 1}, "correlation_id": None,
    }
    receipt = ControllerActionResult("r", "run", "actor", ActionStatus.FAILED, "ROUTE_IMPASSABLE", 0, 4)
    assert asdict(receipt) == {
        "action_request_id": "r", "run_id": "run", "actor_id": "actor",
        "status": "FAILED", "reason_code": "ROUTE_IMPASSABLE", "started_at": 0,
        "resolved_at": 4, "schema_version": 1,
    }
...
def assert_controller_conforms(controller: Controller, observation: Observation) -> None:
    ...
    assert (first.run_id, first.actor_id, first.based_on_observation_id, first.submitted_at) == (
        observation.run_id, observation.actor_id, observation.observation_id, observation.simulation_time,
    )
...
@pytest.mark.parametrize("factory", [ScriptedController, lambda: HumanController(ScriptedController().decide), SocialNpcController])
def test_reusable_controller_conformance_over_live_observations(factory: Callable[[], Controller]) -> None:
...
@pytest.mark.parametrize("case", [
    "none", "dict", "object", "exception", "payload", "cycle", "actor", "run", "observation", "time",
    "version", "unknown", "extra", "bool", "empty_id",
])
def test_controller_failures_cannot_consume_even_due_now_system_events(case: str) -> None:
        ...
        def decide(self, observation: Observation) -> ActionRequest:
            ...
            if case == "payload":
                ...
                changes: dict[str, dict[str, object]] = {
                    "actor": {"actor_id": "b"}, "run": {"run_id": "other"},
                    "observation": {"based_on_observation_id": "unknown"},
                    "time": {"submitted_at": 99}, "version": {"schema_version": 2},
    ...
    try:
        ...
        assert result.failure_code in {
            "CONTROLLER_ERROR", "INVALID_CONTROLLER_OUTPUT", "UNKNOWN_ACTION",
            "INVALID_PAYLOAD", "INVALID_DURATION",
        }
        assert execution(kernel) == before
...
@pytest.mark.parametrize("reason", sorted(PRIVATE_PURCHASE_REASONS | {"PRIVATE", "SELF_TRANSFER", "INVALID_PRICE"}))
@pytest.mark.parametrize("status", [ActionStatus.REJECTED, ActionStatus.FAILED])
def test_receipt_sanitizes_private_purchase_and_uncontracted_diagnostics(reason: str, status: ActionStatus) -> None:
    raw = ActionResult("r", "PRIVATE_HANDLER", None, "PRIVATE_DIGEST", ("PRIVATE_EVENT",), status, reason, 0, 1)
    ...
    assert {field.name for field in fields(receipt)} == {
        "action_request_id", "run_id", "actor_id", "status", "reason_code", "started_at", "resolved_at", "schema_version",
    }


def test_visible_reason_allowlist_is_an_explicit_frozen_contract() -> None:
    assert VISIBLE_DOMAIN_REASONS == frozenset('''
        INVALID_PAYLOAD INVALID_DURATION UNKNOWN_ACTOR INVALID_ENTITY MISSING_POSITION
        INVALID_POSITION UNKNOWN_ROUTE INVALID_ROUTE UNKNOWN_LOCATION INVALID_LOCATION
        WRONG_ORIGIN ROUTE_IMPASSABLE INVALID_TRAVERSAL_COST INVALID_PASSABILITY ROUTE_CHANGED
        SELF_TARGET UNKNOWN_TARGET INVALID_TARGET TARGET_MISSING_POSITION TARGET_INVALID_POSITION
        OUT_OF_RANGE INVALID_QUANTITY UNKNOWN_ITEM UNKNOWN_INVENTORY_OWNER INVALID_INVENTORY_STATE
        INSUFFICIENT_QUANTITY MISSING_SURVIVAL_STATE INVALID_SURVIVAL_STATE ITEM_NOT_CONSUMABLE
        INVALID_WALLET MISSING_WALLET UNKNOWN_OFFER INVALID_OFFER OFFER_UNAVAILABLE SELF_PURCHASE
