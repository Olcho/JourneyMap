...
from journeymap.application.observations import (
    DEFAULT_OBSERVATION_BUDGET_BYTES,
    ObservationHistory,
    ObservationPipeline,
)
...
def test_observation_v1_golden_content_digest_and_section_order() -> None:
    ...
    for order in permutations(((1, "b", "second"), (0, "a", "first"))):
        ...
        for priority, module, contributor in order:
            ...
            pipeline.register(
                priority=priority, module_id=module, contributor_id=contributor, contributor=output
            )
...
def test_exact_utf8_boundary_overflow_and_atomic_history(extra: object) -> None:
    ...
    content: JsonObject = {
        "sections": [{"module_id": "m", "contributor_id": "c", "content": output}]
    }
...
def test_duplicate_visible_identity_cannot_hide_behind_different_priority() -> None:
    ...
    for priority in (0, 1):
        with pytest.raises(ValueError, match="duplicate"):
            pipeline.register(
                priority=priority, module_id="m", contributor_id="c", contributor=lambda c: {}
            )
    ...
    for reverse in (False, True):
        ...
        try:
            ...
            app, research = create_application(
                kernel, initial_knowledge=reversed(records) if reverse else records
            )
            before = execution(kernel)
            game = app.game_for("a")
            if count == 250:
                ...
                assert all(
                    record.knowledge_record_id in canonical_json(observation.content)
        ...
        try:
            ...
            sender.submit(
                ActionRequest(
                    ...
                    "MOVE",
                    1,
                    {"route_id": "bakery-to-village-square"},
                )
            )
